...
const funding_tx_fetcher_1 = __importDefault(require("./funding-tx-fetcher"));
...
class LightningStatsImporter {
    topologiesFolder = config_1.default.LIGHTNING.TOPOLOGY_FOLDER;
    async $run() {
        try {
            const [channels] = await database_1.default.query('SELECT short_id from channels;');
            logger_1.default.info(`Caching funding txs for currently existing channels`, logger_1.default.tags.ln);
            await funding_tx_fetcher_1.default.$fetchChannelsFundingTxs(channels.map(channel => channel.short_id));
            if (config_1.default.MEMPOOL.NETWORK !== 'mainnet' || config_1.default.DATABASE.ENABLED === false) {
                return;
            }
            ...
            logger_1.default.err(`Exception in LightningStatsImporter::$run(). ${e}`);
        }
    }
    /**
     * Generate LN network stats for one day
     */
    async computeNetworkStats(timestamp, networkGraph, isHistorical = false) {
        // Node counts and network shares
        let clearnetNodes = 0;
        let torNodes = 0;
        let clearnetTorNodes = 0;
        let unannouncedNodes = 0;
        const [nodesInDbRaw] = await database_1.default.query(`SELECT public_key FROM nodes`);
        const nodesInDb = {};
        ...
        for (const node of networkGraph.nodes) {
            ...
            if (isHistorical === true && !nodesInDb[node.pub_key]) {
                await nodes_api_1.default.$saveNode({
                ...
                await nodes_api_1.default.$updateNodeSockets(node.pub_key, node.addresses);
            }
            let hasOnion = false;
            ...
            for (const socket of (node.addresses ?? [])) {
                ...
                hasClearnet = hasClearnet || ['ipv4', 'ipv6'].includes(socket.network) || [4, 6].includes((0, net_1.isIP)((socket.addr || '').split(':')[0])) || ((socket.addr || '')).indexOf('ipv4') !== -1 || ((socket.addr || '')).indexOf('ipv6') !== -1;
                ;
            }
        ...
        const [channelsInDbRaw] = await database_1.default.query(`SELECT short_id FROM channels`);
        const channelsInDb = {};
        for (const channel of channelsInDbRaw) {
            channelsInDb[channel.short_id] = channel;
        }
        for (const channel of networkGraph.edges) {
            const short_id = common_1.Common.channelIntegerIdToShortId(channel.channel_id);
            const tx = await funding_tx_fetcher_1.default.$fetchChannelOpenTx(short_id);
            if (!tx) {
                logger_1.default.err(`Unable to fetch funding tx for channel ${short_id}. Capacity and creation date is unknown. Skipping channel.`, logger_1.default.tags.ln);
                continue;
            }
            // If we don't know about this channel, insert it in db
            if (isHistorical === true && !channelsInDb[short_id]) {
                await channels_api_1.default.$saveChannel({
                    channel_id: short_id,
    ...
    `;
      ...
      `;
            ...
            if (!isHistorical) {
                await database_1.default.query(`UPDATE nodes SET capacity = ?, channels = ? WHERE public_key = ?`, [
                    nodeStats[public_key].capacity,
                    nodeStats[public_key].channels,
                    public_key,
                ]);
     ...
     */
    async $importHistoricalLightningStats() {
        if (!config_1.default.LIGHTNING.TOPOLOGY_FOLDER) {
            logger_1.default.info(`Lightning topology folder is not set. Not importing historical LN stats`);
            return;
        }
        logger_1.default.debug('Run the historical importer');
        try {
            let fileList = [];
            try {
                ...
                logger_1.default.err(`Unable to open topology folder at ${this.topologiesFolder}`, logger_1.default.tags.ln);
                throw e;
            }
      ...
      `);
            ...
            for (const filename of fileList) {
                ...
                logger_1.default.debug(`Reading ${this.topologiesFolder}/${filename}`, logger_1.default.tags.ln);
                let fileContent = '';
                try {
                    ...
                    logger_1.default.err(`Unable to open ${this.topologiesFolder}/${filename}`, logger_1.default.tags.ln);
                    totalProcessed++;
                    continue;
                }
                let graph;
                try {
                    ...
                    logger_1.default.debug(`Invalid topology file ${this.topologiesFolder}/${filename}, cannot parse the content. Reason: ${e instanceof Error ? e.message : e}`, logger_1.default.tags.ln);
                    totalProcessed++;
                    continue;
                }
                if (this.isIncorrectSnapshot(timestamp, graph)) {
                    logger_1.default.debug(`Ignoring ${this.topologiesFolder}/${filename}, because we defined it as an incorrect snapshot`);
                    ++totalProcessed;
                    continue;
                }
                if (!logStarted) {
                    logger_1.default.info(`Founds a topology file that we did not import. Importing historical lightning stats now.`, logger_1.default.tags.ln);
                    logStarted = true;
                }
                const datestr = `${new Date(timestamp * 1000).toUTCString()} (${timestamp})`;
                logger_1.default.debug(`${datestr}: Found ${graph.nodes.length} nodes and ${graph.edges.length} channels`, logger_1.default.tags.ln);
                totalProcessed++;
                if (processed > 10) {
                    logger_1.default.info(`Generating LN network stats for ${datestr}. Processed ${totalProcessed}/${fileList.length} files`, logger_1.default.tags.ln);
                    processed = 0;
                }
                else {
                    logger_1.default.debug(`Generating LN network stats for ${datestr}. Processed ${totalProcessed}/${fileList.length} files`, logger_1.default.tags.ln);
                }
                await funding_tx_fetcher_1.default.$fetchChannelsFundingTxs(graph.edges.map(channel => channel.channel_id.slice(0, -2)));
                const stat = await this.computeNetworkStats(timestamp, graph, true);
                existingStatsTimestamps[timestamp] = stat;
            }
            if (totalProcessed > 0) {
                logger_1.default.info(`Lightning network stats historical import completed`, logger_1.default.tags.ln);
            }
        }
        catch (e) {
            logger_1.default.err(`Lightning network stats historical failed. Reason: ${e instanceof Error ? e.message : e}`, logger_1.default.tags.ln);
        }
    }
    cleanupTopology(graph) {
        const newGraph = {
            nodes: [],
            edges: [],
        };
        for (const node of graph.nodes) {
            ...
            for (const address of addressesParts) {
                const formatted = common_1.Common.findSocketNetwork(address);
                addresses.push({
    ...
    isIncorrectSnapshot(timestamp, graph) {
    ...
    async $cleanupIncorrectSnapshot() {
...
exports.default = new LightningStatsImporter;
