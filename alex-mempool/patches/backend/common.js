...
const secp256k1_1 = require("../utils/secp256k1");
...
class Common {
    static nativeAssetId = config_1.default.MEMPOOL.NETWORK === 'liquidtestnet' ?
        '144c654344aa716d6f3abcc1ca90e5641e4e2a7f633bc09fe3baf64585819a49'
        : '6f0279e9ed041c3d710a9f57d0c02928416460c4b722ae3457a11eec381c526d';
    static _isLiquid = config_1.default.MEMPOOL.NETWORK === 'liquid' || config_1.default.MEMPOOL.NETWORK === 'liquidtestnet';
    static isLiquid() {
    ...
    static findRbfTransactions(added, deleted, forceScalable = false) {
        const matches = {};
            ...
            added.forEach((addedTx) => {
                const foundMatches = deleted.filter((deletedTx) => {
                    ...
                    return addedTx.fee > deletedTx.fee
                        ...
                        && deletedTx.vin.some((deletedVin) => addedTx.vin.some((vin) => vin.txid === deletedVin.txid && vin.vout === deletedVin.vout));
                });
    ...
    static findMinedRbfTransactions(minedTransactions, spendMap) {
        const matches = {};
    ...
    static setSchnorrSighashFlags(flags, witness) {
        // no witness items
        if (!witness?.length) {
            return flags;
        }
        ...
        if (witness?.length === (hasAnnex ? 2 : 1)) {
            ...
            if (witness[0].length === 130) {
                ...
                flags |= mempool_interfaces_1.TransactionFlags.sighash_default;
            }
        }
        else {
     ...
     */
    static isNonStandard(tx, height) {
        // version
        ...
        for (const vin of tx.vin) {
            ...
            if (vin.scriptsig_asm) {
                for (const op of vin.scriptsig_asm.split(' ')) {
                    if (bitcoin_script_1.opcodes[op] && bitcoin_script_1.opcodes[op] > bitcoin_script_1.opcodes['OP_16']) {
                        return true;
                    }
                }
            }
            ...
            if (vin.prevout?.scriptpubkey_type === 'p2sh') {
                ...
                const sigops = (transaction_utils_1.default.countScriptSigops(vin.inner_redeemscript_asm) / 4);
        ...
        for (const vout of tx.vout) {
            ...
            if (['nonstandard', 'provably_unspendable', 'empty'].includes(vout.scriptpubkey_type)) {
                ...
                const mOfN = (0, bitcoin_script_1.parseMultisigScript)(vout.scriptpubkey_asm);
            ...
            if (vout.scriptpubkey_type !== 'op_return') {
                ...
                dustSize += (0, bitcoin_script_1.getVarIntLength)(dustSize);
                // add value size
                dustSize += 8;
    ...
    static isWitnessProgram(scriptpubkey) {
    ...
    static isNonStandardVersion(tx, height) {
        let TX_MAX_STANDARD_VERSION = 3;
        if (height != null
            && this.V3_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]
            && height <= this.V3_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]) {
    ...
    static isNonStandardAnchor(tx, height) {
        if (height != null
            && this.ANCHOR_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]
            && height <= this.ANCHOR_STANDARDNESS_ACTIVATION_HEIGHT[config_1.default.MEMPOOL.NETWORK]) {
            // anchor outputs were non-standard to spend before v28.x (scheduled for 2024/09/30 https://github.com/bitcoin/bitcoin/issues/29891)
            return true;
        }
        return false;
    }
    static getNonWitnessSize(tx) {
        ...
        for (const vin of tx.vin) {
            if (vin.witness?.length) {
                hasWitness = true;
                // witness count
                weight -= (0, bitcoin_script_1.getVarIntLength)(vin.witness.length);
                for (const witness of vin.witness) {
                    // witness item size + content
                    weight -= (0, bitcoin_script_1.getVarIntLength)(witness.length / 2) + (witness.length / 2);
                }
            }
        }
        if (hasWitness) {
    ...
    static setSegwitSighashFlags(flags, witness) {
    ...
    static setLegacySighashFlags(flags, scriptsig_asm) {
    ...
    static setSighashFlags(flags, signature) {
        switch (signature.slice(-2)) {
            case '01': return flags | mempool_interfaces_1.TransactionFlags.sighash_all;
            case '02': return flags | mempool_interfaces_1.TransactionFlags.sighash_none;
            case '03': return flags | mempool_interfaces_1.TransactionFlags.sighash_single;
            case '81': return flags | mempool_interfaces_1.TransactionFlags.sighash_all | mempool_interfaces_1.TransactionFlags.sighash_acp;
            case '82': return flags | mempool_interfaces_1.TransactionFlags.sighash_none | mempool_interfaces_1.TransactionFlags.sighash_acp;
            case '83': return flags | mempool_interfaces_1.TransactionFlags.sighash_single | mempool_interfaces_1.TransactionFlags.sighash_acp;
            default: return flags | mempool_interfaces_1.TransactionFlags.sighash_default; // taproot only
        }
    }
    static isBurnKey(pubkey) {
        return [
    ...
    static isInscription(vin, flags) {
        ...
        if (vin.witness.length > (hasAnnex ? 2 : 1)) {
            ...
            const asm = vin.inner_witnessscript_asm || transaction_utils_1.default.convertScriptSigAsm(vin.witness[vin.witness.length - (hasAnnex ? 3 : 2)]);
            // inscriptions smuggle data within an 'OP_0 OP_IF ... OP_ENDIF' envelope
            if (asm?.includes('OP_0 OP_IF')) {
                flags |= mempool_interfaces_1.TransactionFlags.inscription;
            }
        }
        return flags;
    }
    static getTransactionFlags(tx, height) {
        ...
        flags &= ~mempool_interfaces_1.TransactionFlags.cpfp_parent;
        ...
        if (tx.version === 1) {
            flags |= mempool_interfaces_1.TransactionFlags.v1;
        }
        else if (tx.version === 2) {
            flags |= mempool_interfaces_1.TransactionFlags.v2;
        }
        else if (tx.version === 3) {
            flags |= mempool_interfaces_1.TransactionFlags.v3;
        }
        const reusedInputAddresses = {};
        const reusedOutputAddresses = {};
        const inValues = {};
        const outValues = {};
        let rbf = false;
        for (const vin of tx.vin) {
            ...
            if (vin.prevout?.scriptpubkey_type) {
                switch (vin.prevout?.scriptpubkey_type) {
                    case 'p2pk':
                        flags |= mempool_interfaces_1.TransactionFlags.p2pk;
                        break;
                    case 'multisig':
                        flags |= mempool_interfaces_1.TransactionFlags.p2ms;
                        break;
                    case 'p2pkh':
                        flags |= mempool_interfaces_1.TransactionFlags.p2pkh;
                        break;
                    case 'p2sh':
                        flags |= mempool_interfaces_1.TransactionFlags.p2sh;
                        break;
                    case 'v0_p2wpkh':
                        flags |= mempool_interfaces_1.TransactionFlags.p2wpkh;
                        break;
                    case 'v0_p2wsh':
                        flags |= mempool_interfaces_1.TransactionFlags.p2wsh;
                        break;
                    case 'v1_p2tr':
                        {
                            flags |= mempool_interfaces_1.TransactionFlags.p2tr;
                            if (vin.witness?.length) {
            ...
            if (vin.prevout?.scriptpubkey_type === 'v1_p2tr') {
                ...
                flags |= this.setLegacySighashFlags(flags, vin.scriptsig_asm || transaction_utils_1.default.convertScriptSigAsm(vin.scriptsig));
            }
        ...
        if (rbf) {
            flags |= mempool_interfaces_1.TransactionFlags.rbf;
        }
        else {
            flags |= mempool_interfaces_1.TransactionFlags.no_rbf;
        }
        ...
        for (const vout of tx.vout) {
            switch (vout.scriptpubkey_type) {
                case 'p2pk':
                    {
                        ...
                        hasFakePubkey = hasFakePubkey || !(0, secp256k1_1.isPoint)(vout.scriptpubkey?.slice(2, -2));
                    }
                    break;
                case 'multisig':
                    {
                        ...
                        const asm = vout.scriptpubkey_asm || transaction_utils_1.default.convertScriptSigAsm(vout.scriptpubkey);
                        for (const key of (asm?.split(' ') || [])) {
                            if (!hasFakePubkey && !key.startsWith('OP_')) {
                                hasFakePubkey = hasFakePubkey || this.isBurnKey(key) || !(0, secp256k1_1.isPoint)(key);
                            }
                        }
                    }
                    break;
                ...
                case 'v1_p2tr':
                    ...
                    break;
                case 'op_return':
                    flags |= mempool_interfaces_1.TransactionFlags.op_return;
                    break;
            }
            ...
            if (vout.scriptpubkey_type === 'v0_p2wsh') {
                ...
                if (P2WSHCount === Math.ceil((olgaSize + 2) / 32)) {
                    ...
                    if (vout.scriptpubkey.endsWith(''.padEnd(nullBytes * 2, '0'))) {
                        flags |= mempool_interfaces_1.TransactionFlags.fake_scripthash;
                    }
                }
            }
            else {
        ...
        if (hasFakePubkey) {
            flags |= mempool_interfaces_1.TransactionFlags.fake_pubkey;
        }
        ...
        if (!addressReuse && tx.vin.length >= 5 && tx.vout.length >= 5 && (Object.keys(inValues).length + Object.keys(outValues).length) <= (tx.vin.length + tx.vout.length) / 2) {
            flags |= mempool_interfaces_1.TransactionFlags.coinjoin;
        }
        // more than 5:1 input:output ratio
        if (tx.vin.length / tx.vout.length >= 5) {
            flags |= mempool_interfaces_1.TransactionFlags.consolidation;
        }
        // less than 1:5 input:output ratio
        if (tx.vin.length / tx.vout.length <= 0.2) {
            flags |= mempool_interfaces_1.TransactionFlags.batch_payout;
        }
    ...
    static getSimilarity(projectedBlock, transactions) {
        let matchedWeight = 0;
    ...
    static getSqlInterval(interval) {
        switch (interval) {
    ...
    static indexingEnabled() {
        return (['mainnet', 'testnet', 'signet'].includes(config_1.default.MEMPOOL.NETWORK) &&
            config_1.default.DATABASE.ENABLED === true &&
            config_1.default.MEMPOOL.INDEXING_BLOCKS_AMOUNT !== 0);
    }
    static blocksSummariesIndexingEnabled() {
        return (Common.indexingEnabled() &&
            config_1.default.MEMPOOL.BLOCKS_SUMMARIES_INDEXING === true);
    }
    static auditIndexingEnabled() {
        return (Common.indexingEnabled() &&
            config_1.default.MEMPOOL.AUDIT === true);
    }
    static gogglesIndexingEnabled() {
        return (Common.blocksSummariesIndexingEnabled() &&
            config_1.default.MEMPOOL.GOGGLES_INDEXING === true);
    }
    static cpfpIndexingEnabled() {
        return (Common.indexingEnabled() &&
            config_1.default.MEMPOOL.CPFP_INDEXING === true);
    }
    static setDateMidnight(date) {
        date.setUTCHours(0);
        date.setUTCMinutes(0);
        date.setUTCSeconds(0);
        date.setUTCMilliseconds(0);
    }
    static channelShortIdToIntegerId(channelId) {
    ...
    static channelIntegerIdToShortId(id) {
        if (id.indexOf('/') !== -1) {
            id = id.slice(0, -2);
        }
    ...
    static findSocketNetwork(addr) {
        let network = null;
        let url = addr;
        if (config_1.default.LIGHTNING.BACKEND === 'cln') {
            url = addr.split('://')[1];
        }
        if (!url) {
            return {
        ...
        else if (addr.indexOf('ipv4') !== -1 || (config_1.default.LIGHTNING.BACKEND === 'lnd' && (0, net_1.isIP)(url.split(':')[0]) === 4)) {
            const ipv = (0, net_1.isIP)(url.split(':')[0]);
            if (ipv === 4) {
                network = 'ipv4';
            }
            else {
        ...
        else if (addr.indexOf('ipv6') !== -1 || (config_1.default.LIGHTNING.BACKEND === 'lnd' && url.indexOf(']:') !== -1)) {
            if (url.indexOf('[') === -1 || url.indexOf(']') === -1) {
                  return { network: null, url: addr };
              }
              url = url.split('[')[1].split(']')[0];
            const ipv = (0, net_1.isIP)(url);
            if (ipv === 6) {
    ...
    static formatSocket(publicKey, socket) {
        if (config_1.default.LIGHTNING.BACKEND === 'cln') {
            return {
                publicKey: publicKey,
            ...
            const formatted = this.findSocketNetwork((socket && socket.addr) || '');
            return {
                publicKey: publicKey,
    ...
    static calcEffectiveFeeStatistics(transactions) {
        ...
        const halfWidth = config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS / 800;
        const leftBound = Math.floor((config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS / 2) - halfWidth);
        const rightBound = Math.ceil((config_1.default.MEMPOOL.BLOCK_WEIGHT_UNITS / 2) + halfWidth);
        ...
        const minFee = Math.min(Common.getNthPercentile(1, sortedTxs).rate, transactions.slice(-transactions.length / 50).reduce((min, tx) => { return Math.min(min, tx.effectiveFeePerVsize || ((tx.fee || 0) / (tx.weight / 4))); }, Infinity));
        ...
        const maxFee = Math.max(Common.getNthPercentile(99, sortedTxs).rate, transactions.slice(0, transactions.length / 50).reduce((max, tx) => { return Math.max(max, tx.effectiveFeePerVsize || ((tx.fee || 0) / (tx.weight / 4))); }, 0));
        return {
    ...
    static getNthPercentile(n, sortedDistribution) {
        return sortedDistribution[Math.floor((sortedDistribution.length - 1) * (n / 100))];
    }
    static getTransactionFromRequest(req, form) {
        let rawTx = typeof req.body === 'object' && form
    ...
    static getTransactionsFromRequest(req, limit = 25) {
    ...
    static validateTransactionHex(txhex) {
        // Do not mutate txhex
        ...
        if (tx.weight() > config_1.default.MEMPOOL.MAX_PUSH_TX_SIZE_WEIGHT) {
            throw Object.assign(new Error(`Transaction too large (max ${config_1.default.MEMPOOL.MAX_PUSH_TX_SIZE_WEIGHT} weight units)`), { code: -3 });
        }
        // Check 3: Check unreachable script in taproot (if not allowed)
        if (!config_1.default.MEMPOOL.ALLOW_UNREACHABLE) {
            tx.ins.forEach(input => {
                ...
                if (maybeScriptSpend) {
                    ...
                    for (let i = 0; i < decompiled.length - 1; i++) {
                        ...
                        if (first === bitcoinjs.opcodes.OP_FALSE &&
    ...
    static isValidLeafVersion(leafVersion) {
 ...
 */
class OnlineFeeStatsCalculator {
    maxWeight;
    ...
    totalBandFee = 0;
    totalBandWeight = 0;
    minBandRate = Infinity;
    maxBandRate = 0;
    feeRange = [];
    totalWeight = 0;
    constructor(maxWeight, percentileBandWidth, percentiles) {
...
exports.OnlineFeeStatsCalculator = OnlineFeeStatsCalculator;
